...
public:
  ...
  vector<vector<Point>> solution(vector<Point> &vector);
...
public:
  ...
  vector<vector<Point>> Solution::getMinStations(vector<Point> places)
  {
    return solution(places);
  }

  void Solution::initAllPointsAndSolutions(const vector<Point> &vector, std::vector<Point_Item>& points, std::vector<Solution_Item> &allSolutions)
  {
    int max_row = 0;
    int max_column = 0;
  ...
  vector<vector<Point>> Solution::solution(vector<Point> &vector)
  {
    std::vector<Point_Item> points;

    std::vector<Solution_Item> solutions;

    initAllPointsAndSolutions(vector, points, solutions);
    ...
    pocessSinglePoints(points, solutions, usedPoints, usedSolution,
        singleResult);
    int singlePointSize = vector.size() - points.size();
