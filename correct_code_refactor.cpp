...
vector<vector<Point>> getMinStations(vector<Point> places) {
	...
	vector<vector<Point>> result = findApproximateResults(matrix, places, singlePoints);
...
aaa
vector<vector<Point>> findApproximateResults(Matrix& matrix2, const vector<Point>& points, vector<Point>& singlePoints) {
	if (points.empty()) {
		vector<vector<Point>> empty_vector;
		return empty_vector;
	}
